...
export default function EditEventScreen() {
	...
	console.log("Event ID:", searchParams.eventId);

	const isEditing = !!searchParams.eventId;

	const { userId, token } = useAuth();

	const initialTitle = searchParams.title ? String(searchParams.title) : '';
	const initialLocation = searchParams.location ? String(searchParams.location) : '';
	const initialDescription = searchParams.description ? String(searchParams.description) : '';
	const initialStartDate = searchParams.startDate ? new Date(String(searchParams.startDate)) : new Date();
	const initialEndDate = searchParams.endDate ? new Date(String(searchParams.endDate)) : new Date(new Date().setHours(new Date().getHours() + 1));
	const initialImageId = searchParams.imageId ? String(searchParams.imageId) : '';
	...
	const isFormComplete = (isEditing
		? ((image || initialImageId) && title && description && location)
	...
	const handleConfirm = async () => {
		...
		try {
			...
			const response = await fetch(`${API_URL}/${searchParams.eventId}`, {
				method: 'PUT',
				headers: {
			...
			if (!response.ok) {
				...
				throw new Error(`Falha ao atualizar evento. Resposta: ${JSON.stringify(result)}`);
			}
			...
			Alert.alert('Erro', error.message || 'Ocorreu um erro ao atualizar o evento.');
		} finally {
			setLoading(false);
		}
	};
			...
			>
				...
				>
					...
					<TextInput
						style={[
							styles.titleText,
							{ color: title ? '#3C3C3C' : '#B0B0B0', paddingVertical: 12 },
						]}
					...
					<View style={styles.touchableButton}>
						<View style={styles.buttonContent}>
							...
							<DateTimePicker
								...
								display="default"
								themeVariant="light"
								onChange={(event, selectedDate) => selectedDate && handleDateChange('start', selectedDate)}
					...
					<View style={styles.touchableButton}>
						<View style={styles.buttonContent}>
							...
							<DateTimePicker
								...
								display="default"
								themeVariant="light"
								onChange={(event, selectedDate) => selectedDate && handleDateChange('end', selectedDate)}
								minuteInterval={Platform.OS === 'ios' ? 30 : undefined}
								style={{ flex: 1 }}
							/>
						</View>
					...
					<TouchableOpacity style={styles.touchableButton} onPress={() => locationDialogRef.current?.open()}>
						<View style={styles.buttonContent}>
					...
					>
						<View style={[styles.buttonContent, { alignItems: description ? 'flex-start' : 'center' }]}>
							...
							<Text
								style={[styles.buttonText, { color: description ? '#404040' : '#9C9C9C' }]}
								numberOfLines={2}
								ellipsizeMode="tail"
							>
